...
const generarTextoPeriodo = (mes, anio, semana, fecha_inspeccion) => {
  if (semana) {
    if (fecha_inspeccion) return `FECHA DE INSPECCIÓN: ${formatDate(fecha_inspeccion)}`;
...
export async function GET(request) {
  try {
    ...
    let query = `
      ...
      LEFT JOIN Inspecciones_Herramienta ih ON m.id_maquinaria = ih.id_maquinaria
    `;
    const params = [];
    ...
    query += ` WHERE m.area = 'ambiental' AND m.fecha_baja IS NULL `;

    if (id_maquinaria) {
      query += ` AND m.id_maquinaria = ? `;
      params.push(id_maquinaria);
    }

    if (tipoUnidad && tipoUnidad !== 'todos') {
      query += ` AND m.tipo_unidad = ? `;
      params.push(tipoUnidad);
    }
